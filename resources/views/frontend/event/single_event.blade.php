        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12 col-md-12">
                    <h2><b>{{ Str::limit($event->title , 60) }}</b></h2>
                    <ul class="breadcrumb">
                        <li><a href="#"><i class="fas fa-home"></i> Event</a></li>
                                ...
                                <div class="thumb">
                                    <a href="#">
                                        @php
                                            ...
                                            $get_the_file_name_in_array = json_decode($event->image, true);

                                            // If it's not JSON encoded, split the comma-separated string
if ($get_the_file_name_in_array === null) {
    $get_the_file_name_in_array = explode(',', $event->image);
                                            }
                                        @endphp
                                        ...
                                        @foreach ($get_the_file_name_in_array as $value)
                                            @php
                                                ...
                                                $path = 'backend/assets/images/event/' . $file_name;
                                            ...
                                            @if (file_exists(public_path($path)))
                                                <a class="venobox" data-gall="gallery01" href="{{ $path }}">
                                                    <img src="{{ asset($path) }}" style=" padding: 5px;" alt="Event Image">
                                                </a>
                                            @endif
                                        @endforeach
                                    </a>
                                ...
                                <ul>
                                    ...
                                    @foreach ($all_event as $event)
                                        <li>
                                            <div class="info">
                                                <a
                                                    href="{{ url('single-event') }}/{{ $event->slug }}">{{ $event->title }}</a>
                                                <div class="meta-title"><span class="post-date "
                                                        style="color:#ffb606"><b>{{ $event->updated_at }}</b></span>
                                    ...
                                    <div class="text-center">
                                        <a href="{{ url('all-event') }}" class="btn btn-theme effect btn-md"> View All</a>
                                    </div>
